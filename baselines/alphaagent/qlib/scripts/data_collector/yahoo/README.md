...
### Get Qlib data(`bin file`)
  ...
  - parameters:
    - `target_dir`: save dir, by default *~/.qlib/qlib_data/sh_sp500_qlib*
  ...
  - examples:
    ```bash
    # cn 1d
    python scripts/get_data.py qlib_data --target_dir ~/.qlib/qlib_data/sh_sp500_qlib --region cn
    # cn 1min
    python scripts/get_data.py qlib_data --target_dir ~/.qlib/qlib_data/sh_sp500_qlib_1min --region cn --interval 1min
    # us 1d
...
> If the above ready-made data can't meet users' requirements,  users can follow this section to crawl the latest data and convert it to qlib-data.
  1. download data to csv: `python scripts/data_collector/yahoo/collector.py download_data`
     ...
     - examples:
          ```bash
          # cn 1d data
          python collector.py download_data --source_dir ~/.qlib/stock_data/source/sh_sp500_qlib --start 2020-01-01 --end 2020-12-31 --delay 1 --interval 1d --region CN
          # cn 1min data
          python collector.py download_data --source_dir ~/.qlib/stock_data/source/sh_sp500_qlib_1min --delay 1 --interval 1min --region CN
  ...
  2. normalize data: `python scripts/data_collector/yahoo/collector.py normalize_data`
     ...
     - parameters:
      ...
      - examples:
        ```bash
        # normalize 1d cn
        python collector.py normalize_data --source_dir ~/.qlib/stock_data/source/sh_sp500_qlib --normalize_dir ~/.qlib/stock_data/source/cn_1d_nor --region CN --interval 1d

        # normalize 1min cn
        python collector.py normalize_data --qlib_data_1d_dir ~/.qlib/qlib_data/sh_sp500_qlib --source_dir ~/.qlib/stock_data/source/sh_sp500_qlib_1min --normalize_dir ~/.qlib/stock_data/source/cn_1min_nor --region CN --interval 1min
  ...
  3. dump data: `python scripts/dump_bin.py dump_all`
     ...
     - examples:
       ```bash
       # dump 1d cn
       python dump_bin.py dump_all --data_path ~/.qlib/stock_data/source/cn_1d_nor --qlib_dir ~/.qlib/qlib_data/sh_sp500_qlib --freq day --exclude_fields date,symbol --file_suffix .csv
       # dump 1min cn
       python dump_bin.py dump_all --data_path ~/.qlib/stock_data/source/cn_1min_nor --qlib_dir ~/.qlib/qlib_data/sh_sp500_qlib_1min --freq 1min --exclude_fields date,symbol --file_suffix .csv
       ```
...
## Using qlib data
  ...
  qlib.init(provider_uri="~/.qlib/qlib_data/sh_sp500_qlib", region="cn")
  df = D.features(D.instruments("all"), ["$close"], freq="day")

  # 1min data cn
  # freq=1min
  qlib.init(provider_uri="~/.qlib/qlib_data/sh_sp500_qlib_1min", region="cn")
