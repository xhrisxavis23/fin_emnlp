...
class LocalEnv(Env[LocalConf]):
    ...
    def prepare(self):
        if not (Path("~/.qlib/qlib_data/sh_sp500_qlib").expanduser().resolve().exists()):
            self.run(
                entry="python -m qlib.run.get_data qlib_data --target_dir ~/.qlib/qlib_data/sh_sp500_qlib --region cn",
            )
        else:
...
class QlibLocalEnv(LocalEnv):
    ...
    def prepare(self):
        ...
        # 确保Qlib数据目录存在
        qlib_data_path = Path("~/.qlib/qlib_data/sh_sp500_qlib").expanduser()
