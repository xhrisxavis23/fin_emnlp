...
class StockData:
    ...
    def _init_qlib(cls,qlib_path) -> None:
        ...
        # Heuristic: pick region by provider path (sp500/us) if possible.
        region = REG_CN
        try:
            p = str(qlib_path).lower()
    ...
    def _load_exprs(self, exprs: Union[str, List[str]]) -> pd.DataFrame:
        ...
        if cal[end_index] != pd.Timestamp(self._end_time):
            end_index -= 1
        # Clamp end index to calendar length to avoid out-of-bounds when requested end_time
        # is near the end of available data.
    ...
    def _get_data(self) -> Tuple[torch.Tensor, pd.Index, pd.Index]:
        ...
        # If all-NaN (e.g., missing $factor in some datasets), fall back to raw features without factor.
        try:
