...
class RAGEvoAgent(EvoAgent):
    ...
    def multistep_evolve(
        ...
        for _ in tqdm(range(self.max_loop), "Debugging"):
            ...
            # Early stop: if all tasks are successful, there is no need to continue evolving.
            if self.with_feedback and es.feedback is not None:
                try:
                    feedback_list = list(es.feedback)  # type: ignore[arg-type]
                    if feedback_list:
                        all_success = True
                        for fb in feedback_list:
                            if fb is None or getattr(fb, "final_decision", None) is not True:
                                all_success = False
                                break
                        if all_success:
                            break
