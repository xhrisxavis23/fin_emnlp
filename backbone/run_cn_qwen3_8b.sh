...
#
# Prereq:
#   - Start an OpenAI-compatible local server (e.g. vLLM) separately, e.g.
...
# Usage:
#   ./run_cn_qwen3_8b.sh
#   ./run_cn_qwen3_8b.sh "Short-term mean reversion after panic selling" --outer-loop 3
#
# Notes:
#   - This project relies heavily on tool/function calling. Base models may fail.
#     If you see tool calling failures, prefer an Instruct model.
################################################################################
...
# Force local OpenAI-compatible endpoint (avoid accidentally using the real OpenAI API key from .env).
export FINAGENT_OPENAI_BASE_URL="${FINAGENT_OPENAI_BASE_URL:-http://127.0.0.1:8001/v1}"
export OPENAI_BASE_URL="$FINAGENT_OPENAI_BASE_URL"
export FINAGENT_OPENAI_API_KEY="EMPTY"
export OPENAI_API_KEY="EMPTY"

# Model selector (consumed by run/config.py)
