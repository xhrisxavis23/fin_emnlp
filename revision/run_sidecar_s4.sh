...
BATCH_LOG_DIR=$ROOT/favor/runs/_run_logs/_batch_sidecar4_s4_${TS}
: "${OPENAI_API_KEY:?need OPENAI_API_KEY}"
: "${OPENAI_BASE_URL:=https://api.openai.com/v1}"
export OPENAI_API_KEY OPENAI_BASE_URL
SPLITS=("S4|2018-01-01|2020-12-31|2021-01-01|2021-12-31|2022-01-01|2025-12-31")
mkdir -p "$BATCH_LOG_DIR"
JOBS=()
for sd in "${SPLITS[@]}"; do
  for s in $(seq 1 $SEEDS); do JOBS+=("${sd}|${s}"); done
done
echo "[$(date +%H:%M:%S)] SIDECAR4 START" > "$BATCH_LOG_DIR/_progress.log"
launch_one() {
...
echo "[$(date +%H:%M:%S)] SIDECAR4 DONE" >> "$BATCH_LOG_DIR/_progress.log"
