...
# scipy is optional; avoid hard dependency.
def _rank_last_value_average_ties(values: np.ndarray) -> float:
    """
    Equivalent to scipy.stats.rankdata(values, method="average")[-1] for 1-D arrays.
    """
    if values.size == 0:
        return np.nan
    last = values[-1]
    less = float(np.sum(values < last))
    equal = float(np.sum(values == last))
    # average rank among ties (1-based ranks)
...
def rolling_rank(na):
    ...
    """
    na = np.asarray(na)
    return _rank_last_value_average_ties(na)
...
def get_alpha(df):
        ...
        # df['alpha005']=stock.alpha005()  # disabled: buggy (uses python built-in sum)
        ...
        # df['alpha031']=stock.alpha031()  # disabled: uses decay_linear (pandas as_matrix)
        ...
        # df['alpha039']=stock.alpha039()  # disabled: uses decay_linear (pandas as_matrix)
        ...
        # df['alpha057']=stock.alpha057()  # disabled: uses decay_linear (pandas as_matrix)
        df['alpha060']=stock.alpha060()
        df['alpha061']=stock.alpha061()
        df['alpha062']=stock.alpha062()
        # df['alpha064']=stock.alpha064()  # disabled: uses float period in delta()
        df['alpha065']=stock.alpha065()
        # df['alpha066']=stock.alpha066()  # disabled: uses decay_linear (pandas as_matrix)
        # df['alpha068']=stock.alpha068()  # disabled: uses float period in delta()
        # df['alpha071']=stock.alpha071()  # disabled: uses decay_linear + df.at(mask, ...)
        # df['alpha072']=stock.alpha072()  # disabled: uses decay_linear (pandas as_matrix)
        # df['alpha073']=stock.alpha073()  # disabled: uses decay_linear + df.at(mask, ...)
        df['alpha074']=stock.alpha074()
        df['alpha075']=stock.alpha075()
        # df['alpha077']=stock.alpha077()  # disabled: uses decay_linear + df.at(mask, ...)
        ...
        # df['alpha088']=stock.alpha088()  # disabled: uses decay_linear + df.at(mask, ...)
        # df['alpha092']=stock.alpha092()  # disabled: uses decay_linear + df.at(mask, ...)
        df['alpha094']=stock.alpha094()
        # df['alpha095']=stock.alpha095()  # disabled: malformed ts_rank call
        # df['alpha096']=stock.alpha096()  # disabled: uses decay_linear + df.at(mask, ...)
        # df['alpha098']=stock.alpha098()  # disabled: uses decay_linear (pandas as_matrix)
...
class Alphas(object):
    ...
    def alpha005(self):
        raise NotImplementedError(
            "alpha005 disabled: uses python built-in sum(vwap, 10) (likely intended ts_sum/sma); fix before enabling."
        )
...
#                             columns=self.close.columns)
    ...
    def alpha031(self):
        raise NotImplementedError(
            "alpha031 disabled: depends on decay_linear() which uses deprecated/removed pandas.DataFrame.as_matrix()."
        )
        # adv20 = sma(self.volume, 20)
    ...
    def alpha039(self):
        raise NotImplementedError(
            "alpha039 disabled: depends on decay_linear() which uses deprecated/removed pandas.DataFrame.as_matrix()."
        )
        # adv20 = sma(self.volume, 20)
        # return (
        #     (-1 * rank(delta(self.close, 7) * (1 - rank(decay_linear((self.volume / adv20).to_frame(), 9).CLOSE))))
...
#        return (0 - (1 * (rank((sma(self.returns, 10) / sma(sma(self.returns, 2), 3))) * rank((self.returns * self.cap)))))
    ...
    def alpha057(self):
        raise NotImplementedError(
            "alpha057 disabled: depends on decay_linear() which uses deprecated/removed pandas.DataFrame.as_matrix()."
        )
        # return (
        #     0
    ...
    def alpha064(self):
        raise NotImplementedError(
            "alpha064 disabled: calls delta(..., 3.69741) with a float period (diff() expects int)."
        )
        # adv120 = sma(self.volume, 120)
        # return (
        #     (
        #         rank(
        #             correlation(sma(((self.open * 0.178404) + (self.low * (1 - 0.178404))), 13), sma(adv120, 13), 17)
        #         )
        #         < rank(delta(((((self.high + self.low) / 2) * 0.178404) + (self.vwap * (1 - 0.178404))), 3.69741))
        #     )
        #     * -1
        # )
    ...
    def alpha066(self):
        raise NotImplementedError(
            "alpha066 disabled: depends on decay_linear() which uses deprecated/removed pandas.DataFrame.as_matrix()."
        )
        # return (
        #     (
    ...
    def alpha068(self):
        raise NotImplementedError(
            "alpha068 disabled: calls delta(..., 1.06157) with a float period (diff() expects int)."
        )
        # adv15 = sma(self.volume, 15)
        # return (
        #     (ts_rank(correlation(rank(self.high), rank(adv15), 9), 14) < rank(delta(((self.close * 0.518371) + (self.low * (1 - 0.518371))), 1.06157)))
        #     * -1
        # )
    ...
    def alpha071(self):
        raise NotImplementedError(
            "alpha071 disabled: uses decay_linear(as_matrix) and df.at[mask, ...] (at is scalar-only); fix before enabling."
        )
        # adv180 = sma(self.volume, 180)
    ...
    def alpha072(self):
        raise NotImplementedError(
            "alpha072 disabled: depends on decay_linear() which uses deprecated/removed pandas.DataFrame.as_matrix()."
        )
        # adv40 = sma(self.volume, 40)
        # return (
        #     rank(decay_linear(correlation(((self.high + self.low) / 2), adv40, 9).to_frame(), 10).CLOSE)
        #     / rank(decay_linear(correlation(ts_rank(self.vwap, 4), ts_rank(self.volume, 19), 7).to_frame(), 3).CLOSE)
        # )
    
    # Alpha#73	 (max(rank(decay_linear(delta(vwap, 4.72775), 2.91864)),Ts_Rank(decay_linear(((delta(((open * 0.147155) + (low * (1 - 0.147155))), 2.03608) / ((open *0.147155) + (low * (1 - 0.147155)))) * -1), 3.33829), 16.7411)) * -1)
    def alpha073(self):
        raise NotImplementedError(
            "alpha073 disabled: uses decay_linear(as_matrix) and df.at[mask, ...] (at is scalar-only); fix before enabling."
        )
        # p1 = rank(decay_linear(delta(self.vwap, 5).to_frame(), 3).CLOSE)
        # p2 = ts_rank(
        #     decay_linear(
        #         ((delta(((self.open * 0.147155) + (self.low * (1 - 0.147155))), 2) / ((self.open * 0.147155) + (self.low * (1 - 0.147155)))) * -1).to_frame()),
        #         3,
        #     ).CLOSE,
        #     17,
        # )
    ...
    def alpha077(self):
        raise NotImplementedError(
            "alpha077 disabled: uses decay_linear(as_matrix) and df.at[mask, ...] (at is scalar-only); fix before enabling."
        )
        # adv40 = sma(self.volume, 40)
    ...
    def alpha088(self):
        raise NotImplementedError(
            "alpha088 disabled: uses decay_linear(as_matrix) and df.at[mask, ...] (at is scalar-only); fix before enabling."
        )
        # adv60 = sma(self.volume, 60)
    ...
    def alpha092(self):
        raise NotImplementedError(
            "alpha092 disabled: uses decay_linear(as_matrix) and df.at[mask, ...] (at is scalar-only); fix before enabling."
        )
        # adv30 = sma(self.volume, 30)
    ...
    def alpha095(self):
        raise NotImplementedError(
            "alpha095 disabled: malformed ts_rank call (passes a tuple-like '(x, 12)' as the first arg)."
        )
        # adv40 = sma(self.volume, 40)
        # return (
        #     rank((self.open - ts_min(self.open, 12)))
        #     < ts_rank((rank(correlation(sma(((self.high + self.low) / 2), 19), sma(adv40, 19), 13)).pow(5)), 12))
        # )
    
    # Alpha#96	 (max(Ts_Rank(decay_linear(correlation(rank(vwap), rank(volume), 3.83878),4.16783), 8.38151), Ts_Rank(decay_linear(Ts_ArgMax(correlation(Ts_Rank(close, 7.45404),Ts_Rank(adv60, 4.13242), 3.65459), 12.6556), 14.0365), 13.4143)) * -1)
    def alpha096(self):
        raise NotImplementedError(
            "alpha096 disabled: uses decay_linear(as_matrix) and df.at[mask, ...] (at is scalar-only); fix before enabling."
        )
        # adv60 = sma(self.volume, 60)
    ...
    def alpha098(self):
        raise NotImplementedError(
            "alpha098 disabled: depends on decay_linear() which uses deprecated/removed pandas.DataFrame.as_matrix()."
        )
        # adv5 = sma(self.volume, 5)
        # adv15 = sma(self.volume, 15)
        # return (
        #     rank(decay_linear(correlation(self.vwap, sma(adv5, 26), 5).to_frame(), 7).CLOSE)
        #     - rank(decay_linear(ts_rank(ts_argmin(correlation(rank(self.open), rank(adv15), 21), 9), 7).to_frame(), 8).CLOSE))
        # )
