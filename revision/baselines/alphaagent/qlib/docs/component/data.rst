...
After running the above command, users can find china-stock and us-stock data in ``Qlib`` format in the ``~/.qlib/qlib_data/sh_sp500_qlib`` directory and ``~/.qlib/qlib_data/us_data`` directory respectively.
...
for daily data:
  .. code-block:: bash
    ...
    python scripts/get_data.py download_data --file_name csv_data_cn.zip --target_dir ~/.qlib/csv_data/sh_sp500_qlib
...
- You can run the following commands to check whether the data is healthy or not.
    ...
    for daily data:
        .. code-block:: bash
            ...
            python scripts/check_data_health.py check_data --qlib_dir ~/.qlib/qlib_data/sh_sp500_qlib

    for 1min data:
        .. code-block:: bash

            python scripts/check_data_health.py check_data --qlib_dir ~/.qlib/qlib_data/sh_sp500_qlib_1min --freq 1min
...
- You can run the following commands to check whether the data is healthy or not.
    ...
    for daily data:
        .. code-block:: bash
            ...
            python scripts/check_data_health.py check_data --qlib_dir ~/.qlib/qlib_data/sh_sp500_qlib --missing_data_num 30055 --large_step_threshold_volume 94485 --large_step_threshold_price 20

    for 1min data:
        .. code-block:: bash

            python scripts/check_data_health.py check_data --qlib_dir ~/.qlib/qlib_data/sh_sp500_qlib --freq 1min --missing_data_num 35806 --large_step_threshold_volume 3205452000000 --large_step_threshold_price 0.91
...
- If users use ``Qlib`` in china-stock mode, china-stock data is required. Users can use ``Qlib`` in china-stock mode according to the following steps:
    ...
    - Initialize ``Qlib`` in china-stock mode
        Supposed that users download their Qlib format data in the directory ``~/.qlib/qlib_data/sh_sp500_qlib``. Users only need to initialize ``Qlib`` as follows.

        .. code-block:: python

            from qlib.constant import REG_CN
            qlib.init(provider_uri='~/.qlib/qlib_data/sh_sp500_qlib', region=REG_CN)
