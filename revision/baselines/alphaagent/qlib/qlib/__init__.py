...
try:
    ...
    if get_version is None:  # pragma: no cover
        __version__ = "0.0.0"
    else:
    ...
    if conf_path is None:
        ...
        if YAML is None:  # pragma: no cover
            raise ModuleNotFoundError(
                "Missing dependency `ruamel.yaml`. Install it to use `qlib.init_from_yaml_conf`."
            )
...
def auto_init(**kwargs):
    ...
    try:
        ...
        if YAML is None:  # pragma: no cover
            raise ModuleNotFoundError("Missing dependency `ruamel.yaml`. Install it to use `qlib.auto_init`.")
