...
class PortAnaRecord(ACRecordTemp):
    ...
    def _generate(self, **kwargs):
        ...
        for _freq, (report_normal, positions_normal) in portfolio_metric_dict.items():
            ...
            # Save per-day trade records derived from positions.
            #
            # Qlib's backtest returns a `positions_normal` object that is usually a dict:
            #   {datetime -> Position} where Position.position includes instrument holdings and cash.
            # We derive a tabular daily trade log (buy/sell/hold) via `parse_position`.
            try:
                from qlib.contrib.report.analysis_position.parse_position import parse_position

                if isinstance(positions_normal, dict):
                    position_df = parse_position(positions_normal)
                elif isinstance(positions_normal, pd.DataFrame):
                    position_df = positions_normal
                else:
                    position_df = None

                if position_df is not None and "status" in position_df.columns:
                    trade_df = position_df.loc[position_df["status"] != 0].copy()
                    artifact_objects.update({f"trade_records_normal_{_freq}.pkl": trade_df})
            except Exception as e:
                logger.warning(f"Failed to build trade_records_normal_{_freq}.pkl from positions: {e}")
        ...
        for _freq in self.all_freq:
            list_path.extend(
                [
                    ...
                    f"trade_records_normal_{_freq}.pkl",
                ]
            )
        for _analysis_freq in self.risk_analysis_freq:
