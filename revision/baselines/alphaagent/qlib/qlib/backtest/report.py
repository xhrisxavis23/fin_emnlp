...
class PortfolioMetrics:
    ...
    def _cal_benchmark(benchmark_config: Optional[dict], freq: str) -> Optional[pd.Series]:
        ...
        if isinstance(benchmark, pd.Series):
            return benchmark
        elif isinstance(benchmark, str) and benchmark.startswith("market:"):
            # Special syntax: "market:<market_name>"
            # It computes an equal-weighted buy&hold benchmark for that market universe.
            ...
            close_df, _ = get_higher_eq_freq_feature(instruments, ["$close"], start_time, end_time, freq=freq)
            if len(close_df) == 0:
            ...
            universe_cols = prices.loc[base_time].notna()
            prices = prices.loc[:, universe_cols]
            if prices.empty:
                raise ValueError(
                    f'Benchmark market "{market}" has no instruments with prices at {base_time.date().isoformat()}.'
                )
