...
def _compute_equal_weight_buy_and_hold_bench_return(
    ...
    # Include some look-back to compute the first day return (close-to-close).
    # NOTE: `get_date_by_shift` requires `start_time` to be a trading day, but configs may provide a non-trading day
    # (e.g. weekend). Use a calendar-agnostic lookback instead.
    ...
    # Align to report dates and forward-fill to keep value constant after delisting/missing days.
...
class BuyHoldEqualWeightPortAnaRecord(PortAnaRecord):
    ...
    It uses env var `benchmark_list` (comma-separated tickers) as the benchmark universe.
    """
    ...
    def _generate(self, **kwargs):
        ...
        for _freq, (report_normal, positions_normal) in portfolio_metric_dict.items():
            ...
            # Override benchmark with strict buy-and-hold equal-weight series.
        ...
        # Risk analysis + printing (copied from PortAnaRecord, but uses overridden `bench`)
        ...
        # Keep original indicator analysis behavior.
