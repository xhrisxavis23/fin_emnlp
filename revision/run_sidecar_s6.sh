#!/bin/bash
# Sidecar2: S6 only — paper-aligned (3/1/5 shorter train).
# Ensures S6 coverage before 10:00 deadline.
set -u
PARALLEL=4
SEEDS=8
OUTER_LOOP=3
...
BATCH_LOG_DIR=$ROOT/favor/runs/_run_logs/_batch_sidecar2_s6_${TS}
: "${OPENAI_API_KEY:?need OPENAI_API_KEY}"
: "${OPENAI_BASE_URL:=https://api.openai.com/v1}"
export OPENAI_API_KEY OPENAI_BASE_URL
SPLITS=("S6|2017-01-01|2019-12-31|2020-01-01|2020-12-31|2021-01-01|2025-12-31")
mkdir -p "$BATCH_LOG_DIR"
JOBS=()
for sd in "${SPLITS[@]}"; do
  for s in $(seq 1 $SEEDS); do JOBS+=("${sd}|${s}"); done
done
echo "[$(date +%H:%M:%S)] SIDECAR2 START" > "$BATCH_LOG_DIR/_progress.log"
launch_one() {
...
echo "[$(date +%H:%M:%S)] SIDECAR2 DONE" >> "$BATCH_LOG_DIR/_progress.log"
