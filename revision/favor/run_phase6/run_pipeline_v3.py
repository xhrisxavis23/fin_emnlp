#!/usr/bin/env python
"""
Phase 6 pipeline entrypoint v3 — Per-Combo parallel runner with:
  (1) Stage 4 objective branching   (env: FAVOR_STAGE4_OBJECTIVE)
  (2) cfg.qlib.min_cost override    (env: FAVOR_MIN_COST_OVERRIDE)

Diff vs run_pipeline_v2.py — only the cost_patch.apply() call is added.
v2 is kept untouched for backward reproducibility (CSI500 Phase 6 sweep,
A02 multi-seed runs already used it).
...
Env vars (all optional):
    ...
    FAVOR_MIN_COST_OVERRIDE    float (unset → use config.py default)
    STAGE4_COMBO_WORKERS, STAGE4_OPTUNA_N_JOBS

Usage:
    python run_phase6/run_pipeline_v3.py "<concept text>" \\
        --combo-workers 4 --optuna-jobs 1 --outer-loop 3
"""
...
# ============================================================================
# Monkey-patch (2): Phase 6 — _create_objective branching (ir/calmar/ir-λMDD)
# ============================================================================
...
# ============================================================================
# Monkey-patch (3): cost_patch — env-driven cfg.qlib.min_cost override
#   NOTE: must run AFTER stage4_* modules are imported (above) so that their
#   `from run.config import load_rd_config` references get rebound too.
# ============================================================================
from run_phase6.cost_patch import apply as _apply_phase6_cost  # noqa: E402

_cost_patch_info = _apply_phase6_cost()
_cost_override_raw = os.getenv("FAVOR_MIN_COST_OVERRIDE", "")
...
if _cost_override_raw not in ("", None):
    print(f"💰 cost_patch: FAVOR_MIN_COST_OVERRIDE = {_cost_override_raw}")
print("=" * 60)
...
def main() -> None:
    ...
    print("=" * 60)
    print("🚀 Hypothesis-Observation-Validation Framework (Phase 6 v3, Per-Combo Parallel)")
    print("=" * 60)
    ...
    if _cost_override_raw not in ("", None):
        print(f"💰 FAVOR_MIN_COST_OVERRIDE: {_cost_override_raw}")
    if use_outer_loop:
