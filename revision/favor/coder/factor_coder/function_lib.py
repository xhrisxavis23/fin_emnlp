...
def MAX(x: pd.DataFrame, y: pd.DataFrame = None, z: pd.DataFrame = None):
    """
    Overloaded MAX:
    - MAX(A): cross-sectional max by datetime
    - MAX(A, B): elementwise max(A, B)
    - MAX(A, B, C): elementwise max(max(A,B),C)
    """
    if y is None and z is None:
        return x.groupby("datetime").max()
    if y is not None and z is None:
        return np.maximum(x, y)
    if y is not None and z is not None:
        return np.maximum(np.maximum(x, y), z)
    raise ValueError("MAX expects 1, 2, or 3 arguments")
...
def MIN(x: pd.DataFrame, y: pd.DataFrame = None, z: pd.DataFrame = None):
    """
    Overloaded MIN:
    - MIN(A): cross-sectional min by datetime
    - MIN(A, B): elementwise min(A, B)
    - MIN(A, B, C): elementwise min(min(A,B),C)
    """
    if y is None and z is None:
        return x.groupby("datetime").min()
    if y is not None and z is None:
        return np.minimum(x, y)
    if y is not None and z is not None:
        return np.minimum(np.minimum(x, y), z)
    raise ValueError("MIN expects 1, 2, or 3 arguments")
