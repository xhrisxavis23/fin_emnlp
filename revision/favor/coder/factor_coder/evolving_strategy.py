...
from coder.factor_coder.factor import FactorFBWorkspace, FactorTask
...
class FactorMultiProcessEvolvingStrategy(MultiProcessEvolvingStrategy):
    ...
    def implement_one_task(
        ...
        if queried_knowledge is not None and hasattr(queried_knowledge, "task_to_similar_task_successful_knowledge"):
        ...
        if isinstance(queried_knowledge, CoSTEERQueriedKnowledgeV2):
            ...
            if hasattr(queried_knowledge, "task_to_similar_error_successful_knowledge"):
                queried_similar_error_knowledge = queried_knowledge.task_to_similar_error_successful_knowledge.get(
                    target_factor_task_information, []
                )
        else:
            queried_similar_error_knowledge = []
        ...
        if queried_knowledge is not None and hasattr(queried_knowledge, "task_to_former_failed_traces"):
            former = queried_knowledge.task_to_former_failed_traces.get(target_factor_task_information)  # type: ignore[union-attr]
            if isinstance(former, (list, tuple)):
                if len(former) >= 1 and former[0] is not None:
                    queried_former_failed_knowledge = former[0]
                if len(former) >= 2:
                    latest_attempt_to_latest_successful_execution = former[1]
...
class FactorParsingStrategy(MultiProcessEvolvingStrategy):
    ...
    def implement_one_task(
        ...
        # 之前有报错时：提供报错信息和案例给LLM，重写表达式
            ...
            # (may be None when there is no prior successful execution to diff against)
