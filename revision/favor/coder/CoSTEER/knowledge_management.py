"""
================================================================================
CoSTEER Knowledge Management System
================================================================================

[LEGACY NOTICE]
현재 메인 파이프라인(Stage 1-4, run/main.py)에서는 사용되지 않습니다.
향후 통합 가능성을 위해 유지 관리 중입니다.

[관련 파일]
- coder/polars_factor_coder/: CoSTEER 통합 구현 (legacy)
- components/knowledge_management/graph.py: 활성 사용 중인 지식 그래프

[주요 컴포넌트]
- CoSTEERKnowledge: Task + Implementation + Feedback 단위
- CoSTEERKnowledgeBaseV2: 지식 그래프 + 작업 추적
- CoSTEERRAGStrategyV2: 검색 증강 생성 (Former Trace, Component, Error 쿼리)
================================================================================
"""
...
from coder.CoSTEER.evaluators import CoSTEERSingleFeedback
from components.knowledge_management.graph import (
    UndirectedGraph,
    UndirectedNode,
)
...
from core.experiment import FBWorkspace
from core.prompts import Prompts
from core.scenario import Task
from log import logger
from oai.llm_utils import (
    APIBackend,
                ...
                for node in task_des_node_list:
                    for searched_node in self.knowledgebase.graph_query_by_node(
                        ...
                        if searched_node.label != "task_success_implement":
                            continue
                        target_knowledge = self.knowledgebase.node_to_implementation_knowledge_dict.get(searched_node.id)
                ...
                for error_node_list, trace_node in task_trace_node_list:
                    for searched_trace_success_node in self.knowledgebase.graph_query_by_node(
                        ...
                        if (
                            ...
                            trace_knowledge = self.knowledgebase.node_to_implementation_knowledge_dict.get(trace_node.id)
                            success_knowledge = self.knowledgebase.node_to_implementation_knowledge_dict.get(
                                searched_trace_success_node.id
                            )
                            if trace_knowledge is None or success_knowledge is None:
                                # Knowledge store might not have been populated yet (e.g., no successful tasks),
                                # or graph nodes were loaded without their in-memory knowledge payloads.
                                continue
                            error_content = ""
...
class CoSTEERKnowledgeBaseV2(EvolvingKnowledgeBase):
    ...
    def update_success_task(
        ...
        self.graph.add_nodes(
            node=task_des_node,
            neighbors=self.task_to_component_nodes.get(success_task_info, []),
