"""Generate a single-file HTML report for the S&P 500 'US revisit-19' sweep.

Variant of build_report.py: filters runs to the 19 U-series runs (sweep_us_revisit19,
2026-05-13 KST 18:16~22:49) which re-tested CSI500-best settings on S&P 500
(region=us, benchmark=^GSPC, provider=~/.qlib/sh_sp500_qlib).

Output: revision/favor/favor_dashboard_sp.html
...
RUNS_DIR = ROOT / "runs"
OUT = ROOT / "favor_dashboard_sp.html"           # S&P 500 dashboard (separate file)


def _is_us_revisit_run(d: Path) -> bool:
    """Match U-series runs from sweep_us_revisit19 (label like 'U01_...', 'U19_...')."""
    parts = d.name.split("_", 2)
    if len(parts) < 3:
        return False
    head = parts[2].split("_")[0]   # e.g. "U01"
    return len(head) >= 2 and head[0] == "U" and head[1:].isdigit()
...
def main() -> None:
    ...
    html = HTML_HEAD
    html += f"""<header>
  <h1>FaVOR sweep results — S&amp;P 500 (US revisit-19)</h1>
  <p class="meta">Generated {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} · {n_runs} run(s) discovered · {n_complete} complete · sweep_us_revisit19 (2026-05-13)</p>
  <div class="paper-baseline">
    ...
    <div class="caveat">※ paper 의 baseline 은 CSI500. 이 대시보드의 run 들은 동일 셋팅을 S&amp;P500 (region=us, benchmark=^GSPC) 에 적용한 결과 — 시장이 다르므로 직접 비교 X. CSI500 sweep 결과는 favor_dashboard.html 참조.</div>
  </div>
</header>
