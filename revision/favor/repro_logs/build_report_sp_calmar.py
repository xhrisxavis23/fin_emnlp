"""Generate a single-file HTML report for SP500 × A02 × Calmar sweep
(2026-05-15 15:26 ~ 23:29 KST, 20 jobs / 17 unique settings).

Variant of build_report_phase6.py: filters runs to the 20 SP500 sweep jobs
across 3 launcher sh files (mini × 4 concept × 1 seed, gpt-4o × 4 concept +
paper 5seed, gemini-2.5-flash × 4 concept + paper 5seed). A02 setting fixed
(Calmar × stop=None × entry=up_day × pr=0.4 × t=0.55), min_cost=5 (baseline
매칭), MARKET=us.

Run labels:
    SP_M01-M04 (mini × {paper, compressed, volcomp, panic})
    SP_4o_{paper_seed01-05, compressed_seed01, volcomp_seed01, panic_seed01}
    SP_flash_{paper_seed01-05, compressed_seed01, volcomp_seed01, panic_seed01}

Output: revision/favor/favor_dashboard_sp_calmar.html

Original Phase 6 dashboard (build_report_phase6.py → favor_dashboard_phase6.html)
는 CSI500 sweep 결과 보존용으로 그대로 두고, 이 새 dashboard 는 SP500 결과만 표시.
"""
...
RUNS_DIR = ROOT / "runs"
OUT = ROOT / "favor_dashboard_sp_calmar.html"       # SP500 × A02 × Calmar dashboard


_SP_CALMAR_RUN_IDS = None

def _load_phase6_run_ids() -> set:
    """Load run_ids of the SP500 × A02 × Calmar sweep (20 jobs across 3 launchers).

    Function name kept for compatibility with the inherited build_report_phase6.py
    template; semantically it now filters SP500 runs from 3 sweep CSVs.
    """
    global _SP_CALMAR_RUN_IDS
    if _SP_CALMAR_RUN_IDS is not None:
        return _SP_CALMAR_RUN_IDS
    import csv as _csv
    sources = [
        ROOT / "repro_logs" / "sweep_sp500_mini_A02_concepts_results.csv",                   # mini × 4 concept
        ROOT / "repro_logs" / "sweep_sp500_4o_A02_concepts_multiseed_results.csv",           # gpt-4o × 4 concept + paper 5seed
        ROOT / "repro_logs" / "sweep_sp500_flash_A02_concepts_multiseed_results.csv",        # flash × 4 concept + paper 5seed
    ]
    ids = set()
    for p in sources:
        if not p.exists():
        ...
        with open(p) as f:
            for row in _csv.DictReader(f):
                ...
                if rid:
                    ids.add(rid)
    _SP_CALMAR_RUN_IDS = ids
    return ids


def _is_us_revisit_run(d: Path) -> bool:
    """Match SP500 × A02 × Calmar runs by run_id present in the 3 SP500 CSVs."""
    ids = _load_phase6_run_ids()
...
<head>
<meta charset="utf-8">
<title>FaVOR dashboard — S&amp;P 500</title>
<style>
:root {
...
def main() -> None:
    ...
    # Scan run dirs — S&P 500 dashboard only includes U-series (sweep_us_revisit19) runs
    if RUNS_DIR.exists():
        for d in sorted(RUNS_DIR.iterdir()):
            if not d.is_dir():
                continue
            if not _is_us_revisit_run(d):
                continue
            info = collect_run(d)
    ...
    html = HTML_HEAD
    html += f"""<header>
  <h1>FaVOR sweep results — Phase 6 (Calmar objective × wide neighborhood)</h1>
  <p class="meta">Generated {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} · {n_runs} run(s) discovered · {n_complete} complete · sweep_mini_S1_phase6 (2026-05-14 16:51 ~ 2026-05-15 02:40 KST)</p>
  <div class="paper-baseline">
    <strong>Paper Table 1 (CSI 500 FaVOR) 기준선 — 참고용</strong>
    ...
    <div class="caveat">※ Phase 6 sweep — CSI500 × S1 (2y/1y/1y) × gpt-5.4-mini × Stage 4 objective ∈ &#123;ir, calmar, ir_minus_mdd&#125;. honest 4지표 통과 셋팅 1개 (A02 paper_anchor_calmar, IR=+1.92), oracle 통과 4개 (A02, A06, B02, D07). 기존 84-job sweep 대시보드는 favor_dashboard.html.</div>
  </div>
</header>
