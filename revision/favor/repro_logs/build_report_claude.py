"""Generate a single-file HTML report for claude-haiku-4-5 × A02 × Calmar sweep
(2026-05-16, 13 jobs total: CSI500 × paper × 5seed + SP500 × 4concept × paper-5seed-rest-1seed).

Variant of build_report_phase6.py: filters runs to the 13 claude-haiku-4-5 sweep
jobs across 2 launcher sh files. A02 lever fixed (Calmar × stop=None ×
entry=up_day × pr=0.4 × t=0.55), MARKET=cn(csi) or us(sp500, with min_cost=5).

Purpose: 4-backbone (mini, gpt-4o, gemini-flash, claude-haiku) comparison table
의 claude row 채우기. CSI500 paper × 5seed (gpt-4o/flash 와 통일) + SP500 4
concept (paper 5seed + 3 concept 1seed each).

Run labels:
    CSI_claude_paper_A02_seed01-05
    SP_claude_paper_A02_seed01-05
    SP_claude_{compressed, volcomp, panic}_A02_seed01

Output: revision/favor/favor_dashboard_claude.html

Other dashboards (phase6, sp_calmar) 는 그대로 보존.
"""
...
RUNS_DIR = ROOT / "runs"
OUT = ROOT / "favor_dashboard_claude.html"          # claude-haiku-4-5 × A02 × Calmar dashboard


_CLAUDE_RUN_IDS = None

def _load_phase6_run_ids() -> set:
    """Load run_ids of the claude-haiku-4-5 × A02 × Calmar sweep (13 jobs / 2 launchers).

    Function name kept for compatibility with the inherited build_report_phase6.py
    template; semantically it now filters Claude runs from 2 sweep CSVs (csi+sp).
    """
    global _CLAUDE_RUN_IDS
    if _CLAUDE_RUN_IDS is not None:
        return _CLAUDE_RUN_IDS
    import csv as _csv
    sources = [
        ROOT / "repro_logs" / "sweep_csi500_claude_sonnet_A02_paper_multiseed_results.csv",       # CSI500 paper × 5seed = 5 jobs (sonnet-4-6, attempt3 after haiku schema fail)
        ROOT / "repro_logs" / "sweep_sp500_claude_sonnet_A02_concepts_multiseed_results.csv",     # SP500 paper×5seed + 3 concept × 1seed = 8 jobs
    ]
    ids = set()
    for p in sources:
        if not p.exists():
