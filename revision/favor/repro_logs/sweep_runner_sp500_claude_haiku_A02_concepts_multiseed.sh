#!/bin/bash
# ====================================================================
# SP500 × claude-haiku-4-5 × A02 lever × 4 concept (paper 5seed + 3 concept 1seed)
# ====================================================================
# Goal:
#   기존 SP500 A02 sweep (mini/gpt-4o/flash) 의 4 concept 결과와 동일 lever 로
#   claude-haiku-4-5 결과 추가, 4 backbone 비교 table 의 SP500 row 4개 완성:
...
ART_DIR="$LOG_DIR/sweep_artifacts_sp500_claude_haiku_A02_concepts_multiseed"
MASTER_LOG="$LOG_DIR/sweep_sp500_claude_haiku_A02_concepts_multiseed_master.log"
RESULTS_CSV="$LOG_DIR/sweep_sp500_claude_haiku_A02_concepts_multiseed_results.csv"
mkdir -p "$ART_DIR"
...
echo "[$(date +%F\ %T)] SP500 claude-haiku-4-5 A02 × concept multi-seed sweep starting; ${#JOBS[@]} jobs, parallel=$PARALLEL, outer_loop=$OUTER_LOOP" | tee -a "$MASTER_LOG"
...
run_one_job() {
    ...
    local job_log="$LOG_DIR/sweep_sp500_claude_haiku_A02_concepts_multiseed_${label}.log"
    ...
    {
        echo "============================================================"
        echo "[$start_str] [$label] starting (SP500, claude-haiku-4-5, A02, obj=calmar)"
...
wait
echo "[$(date +%F\ %T)] SP500 claude-haiku-4-5 A02 × concept multi-seed sweep complete; ${#JOBS[@]} jobs processed" | tee -a "$MASTER_LOG"
