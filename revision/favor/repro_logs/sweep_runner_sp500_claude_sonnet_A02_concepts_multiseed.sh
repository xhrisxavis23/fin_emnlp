#!/bin/bash
# ====================================================================
# SP500 × claude-sonnet-4-6 × A02 lever × 4 concept (paper 5seed + 3 concept 1seed)
# ====================================================================
# Goal:
#   기존 SP500 A02 sweep (mini/gpt-4o/flash) 의 4 concept 결과와 동일 lever 로
#   claude-sonnet-4-6 결과 추가, 4 backbone 비교 table 의 SP500 row 4개 완성:
#     - paper × sp500     ← multi-seed 5 (gpt-4o/flash 와 통일)
#     - compressed × sp500  ← 1 seed (gpt-4o/flash 와 통일)
#     - volcomp × sp500     ← 1 seed
#     - panic × sp500       ← 1 seed
#
# Plumbing:
#   1) OPENAI_API_KEY=$ANTHROPIC_API_KEY + OPENAI_BASE_URL=$ANTHROPIC_BASE_URL
#   2) MARKET=us → SP500 routing (config.py)
#   3) FAVOR_MIN_COST_OVERRIDE=5 → cost_patch (baseline 매칭)
#
# A02 lever (byte-identical):
#   FAVOR_STAGE4_OBJECTIVE=calmar / STOP=None / ENTRY=up_day / PR=0.4 /
#   THR=0.55~0.95 / N_TRIALS=20 / outer_loop=3
#
# Total: 8 jobs (paper×5 + 3 concept × 1), parallel=5.
# Expected wall ~2-4h. Cost ~$1.5-2.5
# ====================================================================
...
ART_DIR="$LOG_DIR/sweep_artifacts_sp500_claude_sonnet_A02_concepts_multiseed"
MASTER_LOG="$LOG_DIR/sweep_sp500_claude_sonnet_A02_concepts_multiseed_master.log"
RESULTS_CSV="$LOG_DIR/sweep_sp500_claude_sonnet_A02_concepts_multiseed_results.csv"
mkdir -p "$ART_DIR"
...
# ─── BASE env (A02 lever, claude-sonnet-4-6) ──────────────────────────
declare -A BASE=(
    [FAVOR_LLM_MODEL]="claude-sonnet-4-6"
...
# ─── JOBS — paper × 5 seed + 3 concept × 1 seed (총 8) ──────────────
JOBS=()
JOBS+=("SP_claude_paper_A02_seed01|$C_PAPER")
JOBS+=("SP_claude_paper_A02_seed02|$C_PAPER")
JOBS+=("SP_claude_paper_A02_seed03|$C_PAPER")
JOBS+=("SP_claude_paper_A02_seed04|$C_PAPER")
JOBS+=("SP_claude_paper_A02_seed05|$C_PAPER")
JOBS+=("SP_claude_compressed_A02_seed01|$C_COMPRESSED")
JOBS+=("SP_claude_volcomp_A02_seed01|$C_VOLCOMP")
JOBS+=("SP_claude_panic_A02_seed01|$C_PANIC")

PARALLEL=${PARALLEL_JOBS:-1}   # Anthropic 10k output tok/min rate limit 회피 — 순차 실행
OUTER_LOOP=${OUTER_LOOP:-3}
echo "[$(date +%F\ %T)] SP500 claude-sonnet-4-6 A02 × concept multi-seed sweep starting; ${#JOBS[@]} jobs, parallel=$PARALLEL, outer_loop=$OUTER_LOOP" | tee -a "$MASTER_LOG"
echo "[$(date +%F\ %T)] MARKET=$MARKET, provider_uri=$FAVOR_QLIB_PROVIDER_URI_US, OPENAI_BASE_URL=$OPENAI_BASE_URL" | tee -a "$MASTER_LOG"
...
run_one_job() {
    ...
    local job_log="$LOG_DIR/sweep_sp500_claude_sonnet_A02_concepts_multiseed_${label}.log"
    ...
    {
        echo "============================================================"
        echo "[$start_str] [$label] starting (SP500, claude-sonnet-4-6, A02, obj=calmar)"
...
wait
echo "[$(date +%F\ %T)] SP500 claude-sonnet-4-6 A02 × concept multi-seed sweep complete; ${#JOBS[@]} jobs processed" | tee -a "$MASTER_LOG"
