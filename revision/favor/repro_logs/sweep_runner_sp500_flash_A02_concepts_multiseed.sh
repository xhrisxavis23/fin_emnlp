#!/bin/bash
# ====================================================================
# SP500 × gemini-2.5-flash × A02 setting × 4 concept + paper multi-seed
# ====================================================================
# Goal:
#   - paper concept × 5 seed (LLM sampling 변동성 측정)
#   - 나머지 3 concept (compressed / volcomp / panic) × 1 seed
#   gemini-2.5-flash 는 non-reasoning 이라 temperature=0.9 실제 동작.
#   gpt-4o 대비 ~30× 저렴, sanity 통과 (CSI500 multi-seed 결과 일관성 검증됨).
#
# Plumbing:
#   1) MARKET=us → SP500 routing (config.py).
#   2) Gemini OpenAI-호환 endpoint:
#      OPENAI_API_KEY=$GEMINI_API_KEY, OPENAI_BASE_URL=$GEMINI_BASE_URL
#      → OpenAI SDK 가 자동 라우팅, llm_client.py 무수정.
#
...
#
# Layout (8 jobs):
#   paper × seed01-seed05  (5 jobs)
#   compressed × seed01    (1)
#   volcomp    × seed01    (1)
#   panic      × seed01    (1)
#
# Total: 8 jobs, parallel=5. Expected wall ~2-3h. Cost ~$0.48.
# ====================================================================
...
ART_DIR="$LOG_DIR/sweep_artifacts_sp500_flash_A02_concepts_multiseed"
MASTER_LOG="$LOG_DIR/sweep_sp500_flash_A02_concepts_multiseed_master.log"
RESULTS_CSV="$LOG_DIR/sweep_sp500_flash_A02_concepts_multiseed_results.csv"
mkdir -p "$ART_DIR"
...
# load GEMINI_API_KEY / GEMINI_BASE_URL from .env
[ -f "$ROOT/.env" ] && set -a && . "$ROOT/.env" && set +a

# ─── Gemini 라우팅: OpenAI SDK 가 OPENAI_BASE_URL 자동 인식 ───────────
...
# ─── BASE env (A02 setting, gemini-2.5-flash) ────────────────────────
declare -A BASE=(
...
JOBS=()
JOBS+=("SP_flash_paper_A02_seed01|$C_PAPER")
JOBS+=("SP_flash_paper_A02_seed02|$C_PAPER")
JOBS+=("SP_flash_paper_A02_seed03|$C_PAPER")
JOBS+=("SP_flash_paper_A02_seed04|$C_PAPER")
JOBS+=("SP_flash_paper_A02_seed05|$C_PAPER")
JOBS+=("SP_flash_compressed_A02_seed01|$C_COMPRESSED")
JOBS+=("SP_flash_volcomp_A02_seed01|$C_VOLCOMP")
JOBS+=("SP_flash_panic_A02_seed01|$C_PANIC")

PARALLEL=${PARALLEL_JOBS:-5}
OUTER_LOOP=${OUTER_LOOP:-3}
echo "[$(date +%F\ %T)] SP500 gemini-2.5-flash A02 × concept multi-seed sweep starting; ${#JOBS[@]} jobs, parallel=$PARALLEL, outer_loop=$OUTER_LOOP" | tee -a "$MASTER_LOG"
...
run_one_job() {
    ...
    local job_log="$LOG_DIR/sweep_sp500_flash_A02_concepts_multiseed_${label}.log"
    ...
    {
        echo "============================================================"
        echo "[$start_str] [$label] starting (SP500, gemini-2.5-flash, A02, obj=calmar)"
...
wait
echo "[$(date +%F\ %T)] SP500 gemini-2.5-flash A02 × concept multi-seed sweep complete; ${#JOBS[@]} jobs processed" | tee -a "$MASTER_LOG"
