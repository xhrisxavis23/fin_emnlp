"""Generate a single-file HTML report for the Phase 6 sweep (Calmar objective × wide neighborhood).

Variant of build_report.py: filters runs to the 50 Phase 6 jobs
(2026-05-14 16:51 ~ 2026-05-15 02:40 KST) which tested Stage 4 Optuna objective
{ir, calmar, ir_minus_mdd} on CSI500 × S1 (2y/1y/1y) × gpt-5.4-mini.

Run labels: A01-A10, B01-B10, C01-C10, D01-D12, F01-F04, G01-G04
(plus smoke A04_compressed_anchor_calmar from 2026-05-14 16:24).

Output: revision/favor/favor_dashboard_phase6.html

Original CSI500 dashboard (build_report.py → favor_dashboard.html) is untouched.
"""
...
RUNS_DIR = ROOT / "runs"
OUT = ROOT / "favor_dashboard_phase6.html"       # Phase 6 dashboard (separate file)


_PHASE6_RUN_IDS = None

def _load_phase6_run_ids() -> set:
    global _PHASE6_RUN_IDS
    if _PHASE6_RUN_IDS is not None:
        return _PHASE6_RUN_IDS
    import csv as _csv
    sources = [
        ROOT / "repro_logs" / "sweep_mini_S1_phase6_results.csv",            # 원본 50-job sweep
        ROOT / "repro_logs" / "sweep_a02_4o_multiseed_results.csv",          # A02 × gpt-4o multi-seed 검증
        ROOT / "repro_logs" / "sweep_a02_gemini_flash_multiseed_results.csv", # A02 × gemini-2.5-flash multi-seed 검증
    ]
    ids = set()
    for p in sources:
        if not p.exists():
...
def _is_us_revisit_run(d: Path) -> bool:
    """Match Phase 6 runs by run_id present in sweep_mini_S1_phase6_results.csv.

    Function name kept for compatibility with the original build_report_sp.py
    template; semantically it now filters Phase 6 runs (Calmar objective sweep,
    2026-05-14 16:24 ~ 2026-05-15 02:40 KST, 50 jobs + 1 smoke re-used).
    """
