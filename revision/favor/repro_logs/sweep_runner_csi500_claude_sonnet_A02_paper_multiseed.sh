#!/bin/bash
# ====================================================================
# CSI500 × claude-sonnet-4-6 × A02 (paper concept) × 5 seed
# ====================================================================
# Goal:
#   기존 A02 paper × CSI500 multi-seed 결과 (mini single +1.92, gpt-4o 5seed
#   mean=-0.667, flash 5seed mean=-1.212) 와 동일 lever 로 claude-sonnet-4-6
...
ART_DIR="$LOG_DIR/sweep_artifacts_csi500_claude_sonnet_A02_paper_multiseed"
MASTER_LOG="$LOG_DIR/sweep_csi500_claude_sonnet_A02_paper_multiseed_master.log"
RESULTS_CSV="$LOG_DIR/sweep_csi500_claude_sonnet_A02_paper_multiseed_results.csv"
mkdir -p "$ART_DIR"
...
echo "[$(date +%F\ %T)] CSI500 claude-sonnet-4-6 A02 × paper multi-seed sweep starting; ${#JOBS[@]} jobs, parallel=$PARALLEL, outer_loop=$OUTER_LOOP" | tee -a "$MASTER_LOG"
...
run_one_job() {
    ...
    local job_log="$LOG_DIR/sweep_csi500_claude_sonnet_A02_paper_multiseed_${label}.log"
    ...
    {
        echo "============================================================"
        echo "[$start_str] [$label] starting (CSI500, claude-sonnet-4-6, A02, obj=calmar)"
...
wait
echo "[$(date +%F\ %T)] CSI500 claude-sonnet-4-6 A02 × paper multi-seed sweep complete; ${#JOBS[@]} jobs processed" | tee -a "$MASTER_LOG"
